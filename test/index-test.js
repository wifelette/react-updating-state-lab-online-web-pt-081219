...
describe("<DigitalClicker />", () => {
  it("should have the correct initial state", () => {
    ...
    expect(wrapper.state().timesClicked).to.not.equal(
      undefined,
      "The `timesClicked` property does not exist in the state"
    );
    expect(wrapper.state().timesClicked)
      .to.be.a("number")
      .to.equal(0, "The `timesClicked` property does have the right value");
  });
